# 7.33:  Modify selected lines from a list using a list
# comprehension and a compound statement.

# Write a list comprehension that returns a list of last names
# only from 'Acme Inc.' lines.

import runreport

lines = [  '2014-03-09,Wilson,Joe,Acme Inc.',
           '2014-08-17,Fink,Bart,Beta LLC',
           '2018-09-03,Emerson,Will,Acme Inc.',
           '2020-12-09,Rodgers,Mary,Gamma Co.',
           '2014-07-01,Jones,Pete,Acme Inc.'
        ]



# Expected Output:

# ['Wilson', 'Emerson', 'Jones']

